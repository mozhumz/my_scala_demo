 ...
 *
 * 分析：
 * 1 将主从物品看作一组，该组的编号可定义为行号i（i从1开始）
 * 如果输入有m个物品数，则可以分为m组，显然如果输入有从物品，则某些组的主从物品权重和价格均为0
 * 2 定义dp[i][j] 在给定j金额和i物品数时的最大权值
 *  设V[i][0] V[i][1] V[i][2] 分别为编号为i组的“主从1从2”物品价格
 *  设P[i][0] P[i][1] P[i][2] 分别为编号为i组的“主从1从2”物品权重
 * 3 在给定ij时，此时已经知到dp[i-1][j]和dp[i][j-1]的初始值，
 *   要求dp[i][j]，则需要做出权值最大的选择：
 *   max(不选择第i组物品时的权值，选择主物品的权值，选择主从1的权值，选择主从2的权值,选择主从1从2的权值)
 *
 */
